...
if [ -z "$1" ]; then
  ...
  echo "e.g."
  echo "./openjdk.sh <VERSION> [port]"
  echo "VERSION e.g. 20"
  echo "port default is 8888 (needed for some demos)"
  exit 1
fi
...
if [ "$1" == "16" ]; then
  ...
  docker run -it --rm -p ${2:-8888}:8888 -v "$(pwd)/src:/src" openjdk:$1-slim /bin/bash -c "cd /src/java$1;ls;pwd;export JAVA_OPTS='--enable-preview --source $1';echo \"export PS1='$ '\" >>/root/.bashrc;printenv;java -version;exec /bin/bash"
fi
