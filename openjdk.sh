...
if [ "$1" == "16" ]; then
  ...
  docker run -it --rm -v "$(pwd)/src:/src" openjdk:$1-slim /bin/bash -c "cd /src/java$1;ls;pwd;export JAVA_OPTS='--enable-preview --source $1';export PS1='$ ';java -version;exec /bin/bash"
fi
